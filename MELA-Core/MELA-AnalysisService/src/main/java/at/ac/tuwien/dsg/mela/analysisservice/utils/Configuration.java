...
public class Configuration {
	...
	static {
		...
		try {
			InputStream is = new FileInputStream(
					new File(
							System.getProperty("at.ac.tuwien.dsg.mela.configuration.file")));
			configuration.load(is);

			PropertyConfigurator
					.configure(new FileInputStream(
							new File(
									System.getProperty("at.ac.tuwien.dsg.mela.configuration.logfile"))));
			logger = Logger.getLogger("melaAnalysisServiceLogger");
		} catch (Exception ex) {
			Logger.getLogger(Configuration.class.getName()).log(Level.FATAL,
					null, ex);
		}
	...
	public static int getMonitoringAggregationIntervalInSeconds() {
		if (configuration.containsKey("DATA_COLLECTION_INTERVAL_IN_SECONDS")) {
			return Integer.parseInt(configuration
					.getProperty("DATA_COLLECTION_INTERVAL_IN_SECONDS"));
		} else {
	...
	public static int getMonitoringIntervalInSeconds() {
		if (configuration.containsKey("DATA_AGGREGATION_WINDOWS")) {
			return Integer.parseInt(configuration
					.getProperty("DATA_AGGREGATION_WINDOWS"));
		} else {
	...
	public static Boolean isElasticityAnalysisEnabled() {
		if (configuration.containsKey("ELASTICITY_ANALYSIS_ENABLED")) {
			return Boolean.parseBoolean(configuration.getProperty(
					"ELASTICITY_ANALYSIS_ENABLED").toLowerCase());
		} else {
			return true; // default
		}
	}
	...
	public static int getDataServicePort() {
		if (configuration.containsKey("MELA_DATA_SERVICE_PORT")) {
			return Integer.parseInt(configuration
					.getProperty("MELA_DATA_SERVICE_PORT"));
		} else {
