...
public class SystemControl {
	...
	private int monitoringIntervalInSeconds = Configuration
			.getMonitoringIntervalInSeconds();
	...
	private int aggregationWindowsCount = Configuration
			.getMonitoringAggregationIntervalInSeconds();
	...
	private Boolean isElasticityEnabled = Configuration
			.isElasticityAnalysisEnabled();
	// private AggregatedMonitoringDataSQLAccess
	// aggregatedMonitoringDataSQLAccess;
	// used in monitoring
	...
	protected SystemControl() {
		...
		// aggregatedMonitoringDataSQLAccess = new
		// AggregatedMonitoringDataSQLAccess(
		// "mela", "mela");
	}
	...
	public synchronized void addExecutingAction(String targetEntityID,
			String actionName) {
		MonitoredElement element = new MonitoredElement(targetEntityID);
		actionsInExecution.put(element, actionName);
	}

	public synchronized void removeExecutingAction(String targetEntityID,
			String actionName) {
		MonitoredElement element = new MonitoredElement(targetEntityID);
		if (actionsInExecution.containsKey(element)) {
			actionsInExecution.remove(element);
		} else {
			Configuration.getLogger(this.getClass()).log(
					Level.INFO,
					"Action " + actionName + " on monitored element "
							+ targetEntityID + " not found.");
		}
	}
	...
	public synchronized void setServiceConfiguration(
			MonitoredElement serviceConfiguration) {
		this.serviceConfiguration = serviceConfiguration;
		elasticitySpaceFunction = new ElSpaceDefaultFunction(
	...
	// adds new ones.
	public synchronized void updateServiceConfiguration(
			MonitoredElement serviceConfiguration) {
		...
		for (MonitoredElement element : serviceConfiguration) {
			if (serviceUnits.containsKey(element)) {
				...
				serviceUnits.get(element).getContainedElements()
						.addAll(element.getContainedElements());
			}
		}
	...
	public synchronized void setCompositionRulesConfiguration(
		...
		if (dataAccess != null) {
			...
			// rule
			for (CompositionRule compositionRule : compositionRulesConfiguration
					.getMetricCompositionRules().getCompositionRules()) {
				...
				while (!queue.isEmpty()) {
					...
					// metric can be null if a composition rule artificially
					// creates a metric using SET_VALUE
					if (targetMetric != null) {
						MetricFilter metricFilter = new MetricFilter();
						metricFilter.setId(targetMetric.getName() + "_Filter");
						metricFilter.setLevel(operation
								.getMetricSourceMonitoredElementLevel());
			...
			Configuration
					...
					.log(Level.WARN,
							"Data Access source not set yet on SystemControl."
	...
	public synchronized ServiceMonitoringSnapshot getAggregatedMonitoringDataOverTime(
			List<ServiceMonitoringSnapshot> serviceMonitoringSnapshots) {
		if (serviceMonitoringSnapshots.size() > 1) {
			return instantMonitoringDataEnrichmentEngine
					.aggregateMonitoringDataOverTime(
							compositionRulesConfiguration,
							serviceMonitoringSnapshots);
		} else {
			return instantMonitoringDataEnrichmentEngine.enrichMonitoringData(
					compositionRulesConfiguration,
					serviceMonitoringSnapshots.get(0));
		}
	}

	private synchronized ServiceMonitoringSnapshot getAggregatedMonitoringData(
			ServiceMonitoringSnapshot rawMonitoringData) {
		if (dataAccess != null) {
			return instantMonitoringDataEnrichmentEngine.enrichMonitoringData(
					compositionRulesConfiguration, rawMonitoringData);
		} else {
			Configuration.getLogger(this.getClass()).log(Level.WARN,
					"Data Access source not set yet on SystemControl");
			return new ServiceMonitoringSnapshot();
	...
	private synchronized AnalysisReport analyzeAggregatedMonitoringData(
			ServiceMonitoringSnapshot rawMonitoringData) {
		if (dataAccess != null) {
			return instantMonitoringDataAnalysisEngine.analyzeRequirements(
					instantMonitoringDataEnrichmentEngine.enrichMonitoringData(
							compositionRulesConfiguration, rawMonitoringData),
					requirements);
		} else {
			Configuration.getLogger(this.getClass()).log(Level.WARN,
					"Data Access source not set yet on SystemControl");
			return new AnalysisReport(new ServiceMonitoringSnapshot(),
					new Requirements());
		}
	}
	...
	public synchronized AnalysisReport analyzeLatestMonitoringData() {
		if (dataAccess != null) {
			return instantMonitoringDataAnalysisEngine.analyzeRequirements(
					instantMonitoringDataEnrichmentEngine
							.enrichMonitoringData(
									compositionRulesConfiguration,
									latestMonitoringData), requirements);
		} else {
			Configuration.getLogger(this.getClass()).log(Level.WARN,
					"Data Access source not set yet on SystemControl");
			return new AnalysisReport(new ServiceMonitoringSnapshot(),
	...
	public synchronized Collection<Metric> getAvailableMetricsForMonitoredElement(
			MonitoredElement MonitoredElement) throws ConfigurationException {
		if (dataAccess != null) {
			return dataAccess
					.getAvailableMetricsForMonitoredElement(MonitoredElement);
		} else {
			Configuration.getLogger(this.getClass()).log(Level.WARN,
					"Data Access source not set yet on SystemControl");
	...
	public synchronized void addMetricFilters(
			Collection<MetricFilter> newFilters) {
		if (dataAccess != null) {
	...
	public synchronized void removeMetricFilters(
			Collection<MetricFilter> filtersToRemove) {
		if (dataAccess != null) {
	...
	public synchronized void setMonitoringIntervalInSeconds(
			int monitoringIntervalInSeconds) {
		this.monitoringIntervalInSeconds = monitoringIntervalInSeconds;
	}

	public synchronized void setNrOfMonitoringWindowsToAggregate(
			...
			public void run() {
				if (serviceConfiguration != null) {
					...
					if (monitoringData != null) {
						...
						// if we have no composition function, we have no
						// metrics, so it does not make sense to train the
						// elasticity space
						// if (isElasticityEnabled
						// && compositionRulesConfiguration != null) {
						// // write monitoring data in sql
						// aggregatedMonitoringDataSQLAccess
						// .writeMonitoringData(latestMonitoringData);
						// //
						...
						// this.cancel();
						Configuration.getLogger(this.getClass()).log(
								Level.ERROR, "Monitoring data is NULL");
					}
				} else {
					Configuration.getLogger(this.getClass()).log(Level.WARN,
	...
	public synchronized String getElasticityPathwayLazy(MonitoredElement element) {
		...
		// if (!Configuration.isElasticityAnalysisEnabled()
		// || serviceConfiguration == null
		// && compositionRulesConfiguration != null) {
		// Configuration
		// .getLogger(this.getClass())
		// .log(Level.WARN,
		// "Elasticity analysis disabled, or no service configuration or composition rules configuration");
		// }
		//
		// // int recordsCount =
		// // aggregatedMonitoringDataSQLAccess.getRecordsCount();
		//
		// // first, read from the sql of monitoring data, in increments of 10,
		// and
		// // train the elasticity space function
		// LightweightEncounterRateElasticityPathway elasticityPathway = null;
		//
		// ElasticitySpace tempSpace = new
		// ElasticitySpace(serviceConfiguration);
		...
		// aggregatedMonitoringDataSQLAccess
		// .extractMonitoringData(i * 500, 500);
		...
		// }
		// }
		// Map<Metric, List<MetricValue>> map = tempSpace
		// .getMonitoredDataForService(element);
		...
		// elasticityPathway = new LightweightEncounterRateElasticityPathway(
		// metrics.size());
		// }
		//
		...
		// Configuration.getLogger(this.getClass()).log(
		// Level.ERROR,
		// "Service Element " + element.getId() + " at level "
		// + element.getLevel()
	...
	public synchronized String getElasticitySpaceLazy(MonitoredElement element) {
		...
		return elSpaceJSON.toJSONString();
		// // if no service configuration, we can't have elasticity space
		// function
		// // if no compositionRulesConfiguration we have no data
		// if (!Configuration.isElasticityAnalysisEnabled()
		// || serviceConfiguration == null
		...
		// elasticitySpaceFunction
		// .trainElasticitySpace(monitoringSnapshot);
		// }
		// }
		// }
		//
		// String jsonRepr = ConvertToJSON.convertElasticitySpace(
		// elasticitySpaceFunction.getElasticitySpace(), element);
	...
	public synchronized String getElasticitySpace(MonitoredElement element) {
		...
		// .extractMonitoringData();
		// if (extractedData != null) {
		// // for each extracted snapshot, trim it to contain data only for the
		// // targetedMonitoredElement (minimizes RAM usage)
		// for (ServiceMonitoringSnapshot monitoringSnapshot : extractedData) {
		// // monitoringSnapshot.keepOnlyDataForElement(element);
		// elasticitySpaceFunction
		// .trainElasticitySpace(monitoringSnapshot);
	...
	public synchronized String getLatestMonitoringDataINJSON() {
		return ConvertToJSON.convertMonitoringSnapshot(latestMonitoringData,
				requirements, actionsInExecution);
	}

	public synchronized String getMetricCompositionRules() {
		if (compositionRulesConfiguration != null) {
			return ConvertToJSON.convertToJSON(compositionRulesConfiguration
					.getMetricCompositionRules());
		} else {
