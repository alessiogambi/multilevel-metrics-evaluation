...
import at.ac.tuwien.dsg.mela.dataservice.dataSource.impl.DummyDataSource;
import at.ac.tuwien.dsg.mela.dataservice.dataSource.impl.GangliaSQLDataSource;
import at.ac.tuwien.dsg.mela.dataservice.dataSource.impl.LocalGangliaLiveDataSource;
...
public class DataAccesForTestsOnly extends AbstractDataAccess {
	...
	public static DataAccesForTestsOnly createInstance() {
		...
		if (accessType.equalsIgnoreCase("LocalGanglia")) {
			...
			Configuration
					.getLogger(DataAccesForTestsOnly.class)
					.log(Priority.ERROR,
	...
	public synchronized ServiceMonitoringSnapshot getMonitoredData(
		...
		try {
			...
			Configuration.getLogger(this.getClass()).log(Level.ERROR,
					e.getMessage(), e);
			Configuration.getLogger(this.getClass()).log(Level.ERROR,
		...
		for (HostInfo gangliaHostInfo : gangliaHostsInfo) {
			...
			for (MetricInfo gangliaMetricInfo : gangliaHostInfo.getMetrics()) {
				...
				if (metric.getName().equals(
						Configuration.getMonitoredElementIDMetricName())) {
					monitoredElement = new MonitoredElement();
					monitoredElement.setId(gangliaMetricInfo.getValue());
					monitoredElement
							.setLevel(MonitoredElement.MonitoredElementLevel.SERVICE_UNIT);
				}
			}
			...
			if (monitoredElement != null
				...
				MonitoredElementMonitoringSnapshot MonitoredElementMonitoringSnapshot = new MonitoredElementMonitoringSnapshot(
						vmLevelElement, monitoredMetricValues);
	...
	public synchronized MonitoredElementMonitoringSnapshot getSingleElementMonitoredData(
		...
		for (HostInfo gangliaHostInfo : gangliaHostsInfo) {
			...
			// logic under our ganglia deployment so far)
			if (monitoredElement != null
					&& MonitoredElement.equals(monitoredElement)) {

				MonitoredElementMonitoringSnapshot monitoredElementMonitoringSnapshot = new MonitoredElementMonitoringSnapshot(
						MonitoredElement, monitoredMetricValues);
