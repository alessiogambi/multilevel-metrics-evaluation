...
public class DataAccess extends AbstractDataAccess {
	...
	public static DataAccess createInstance() {
		...
		if (accessType.equalsIgnoreCase("LocalGanglia")) {
			Configuration.getLogger(DataAccess.class).log(Level.INFO,
					"Using Local Ganglia data source");
			...
			Configuration.getLogger(DataAccess.class).log(Level.INFO,
					"Using Remote Ganglia data source");
			DataSourceI dataSource = new RemoteGangliaLiveDataSource();
			return new DataAccess(dataSource);
		} else if (accessType.equalsIgnoreCase("Replay")) {
			String monitoringSeqID = Configuration
					.getStoredMonitoringSequenceID();
			DataSourceI dataSourceI = new GangliaSQLDataSource(monitoringSeqID,
					"mela", "mela");
			return new DataAccess(dataSourceI);
		} else {
			Configuration
					.getLogger(DataAccess.class)
					.log(Priority.ERROR,
							"MELA-DataService data access mode not specified or not recognized");
	 ...
	 */
	@Override
	public synchronized ServiceMonitoringSnapshot getMonitoredData(
			MonitoredElement m) {

		if (m == null) {
			Configuration.getLogger(DataAccess.class).log(Level.WARN,
					"No supplied service configuration");
		...
		try {
			...
			Configuration.getLogger(DataAccess.class).log(Level.ERROR,
					e.getMessage(), e);
			Configuration.getLogger(DataAccess.class).log(Level.ERROR,
		...
		MonitoredElementMonitoringSnapshot rootMonitoringSnapshot = new MonitoredElementMonitoringSnapshot(
				structureRoot, new LinkedHashMap<Metric, MetricValue>());
		...
		while (!bfsTraversalQueue.isEmpty()) {
			MonitoredElementMonitoringSnapshot element = bfsTraversalQueue
					.remove(0);
			...
			for (MonitoredElement child : processedElement
					.getContainedElements()) // add empty monitoring data for
												// each serviceStructure
												// element, to serve as a place
												// where in the future composed
												// metrics can be added
			{
				MonitoredElementMonitoringSnapshot monitoredElementMonitoringSnapshot = new MonitoredElementMonitoringSnapshot(
						child, new LinkedHashMap<Metric, MetricValue>());
				element.addChild(monitoredElementMonitoringSnapshot);
				serviceMonitoringSnapshot
						.addMonitoredData(monitoredElementMonitoringSnapshot);
				bfsTraversalQueue.add(monitoredElementMonitoringSnapshot);
			}

		}

		// iterate trough the GangliaCluster, extract each VM monitoring data,
		// build an MonitoredElementMonitoringSnapshot from it and add it to the
		// ServiceMonitoringSnapshot

		Collection<HostInfo> gangliaHostsInfo = gangliaClusterInfo
				.getHostsInfo();
		...
		for (HostInfo gangliaHostInfo : gangliaHostsInfo) {
			...
			// MonitoredElementMonitoringSnapshot
			// MonitoredElementMonitoringSnapshot = new
			// MonitoredElementMonitoringSnapshot();

			// currently we assume for VMs that their ID is their IP (as this si
			// what is unique for them in a single cloud deployment on the same
			// network space)
			MonitoredElement monitoredElement = new MonitoredElement(
					gangliaHostInfo.getIp());
			monitoredElement
					.setLevel(MonitoredElement.MonitoredElementLevel.VM);
			...
			for (MetricInfo gangliaMetricInfo : gangliaHostInfo.getMetrics()) {
				...
				MetricValue metricValue = new MetricValue(
						gangliaMetricInfo.getConvertedValue());
			...
			if (monitoredElement != null
				...
				MonitoredElementMonitoringSnapshot monitoredElementMonitoringSnapshot = new MonitoredElementMonitoringSnapshot(
						structureElement, monitoredMetricValues);

				// also add VM monitoring info to children tree
				serviceMonitoringSnapshot.getMonitoredData(monitoredElement)
						.addChild(monitoredElementMonitoringSnapshot);
		...
		// such exist
		// the filtering is done here after collections since I iterate trough
		// all metrics above to find the MonitoredElementID
		// which is later used to determine to which level I need to map the
		// data
		// also i can use the code below when I get data at diff levels and I
		// move from Ganglia
	...
	@Override
	public synchronized MonitoredElementMonitoringSnapshot getSingleElementMonitoredData(
			MonitoredElement suppliedMonitoringElement) {
		...
		for (HostInfo gangliaHostInfo : gangliaHostsInfo) {
			...
			// logic under our ganglia deployment so far)
			if (monitoredElement != null
					&& suppliedMonitoringElement.equals(monitoredElement)) {

				MonitoredElementMonitoringSnapshot monitoredElementMonitoringSnapshot = new MonitoredElementMonitoringSnapshot(
						suppliedMonitoringElement.clone(),
						monitoredMetricValues);

				// filters are applied sequentially in cascade
				if (metricFilters.containsKey(suppliedMonitoringElement
						.getLevel())) {
					for (MetricFilter filter : metricFilters
							.get(suppliedMonitoringElement.getLevel())) {
						// if either the filter applies on all elements at one
						// particular level (targetIDs are null or empty) either
						// the filter targets the serviceStructure element ID
						if (filter.getTargetMonitoredElementIDs() == null
								|| filter.getTargetMonitoredElementIDs().size() == 0
								|| filter.getTargetMonitoredElementIDs()
										.contains(monitoredElement.getId())) {
							monitoredElementMonitoringSnapshot
									.keepMetrics(filter.getMetrics());
						}
					}
				}
	...
	@Override
	public Collection<Metric> getAvailableMetricsForMonitoredElement(
			MonitoredElement suppliedMonitoringElement) {
		ClusterInfo gangliaClusterInfo = null;
		try {
			gangliaClusterInfo = gangliaDataSourceI.getMonitoringData();
